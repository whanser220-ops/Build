Shader "New_Grass/BezierBladeToonMid"
{
    Properties
    {
        [Header(Color)]
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            void GetP1P2P3(float3 p0, inout float3 p3, float bend, float hash, float windForce, out float3 p1, out float3 p2)
            {
                ...
                float3 bezCtrlOffsetDir = normalize(cross(bladeDir, float3(0.0, 0.0, 1.0)));
            ...
            Varyings Vert(Attributes IN)
            {
                ...
                float3x3 rotMat = RotateY(-rotationAngle);
            ...
            half4 Frag(Varyings IN) : SV_Target
            {
                ...
                float3 ambient = SampleSH(n) * albedo * _AmbientStrength;
