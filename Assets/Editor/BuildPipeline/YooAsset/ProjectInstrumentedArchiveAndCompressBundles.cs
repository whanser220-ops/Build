...
#pragma warning restore 649
    ...
    public ReturnCode Run()
    {
        TaskInput input = new TaskInput
        {
            ...
            Threaded = ReflectionExtensions.SupportsMultiThreadedArchiving && SbpPreferences.threadedArchiving
        };
        ...
        List<SerializedFileMetaData> hashes)
    {
        CacheEntry entry = new CacheEntry
        {
            ...
            Guid = HashingMethods.Calculate("ArchiveAndCompressBundles", bundleName).ToGUID()
        };
        ...
        entry.Version = TaskVersion;
        return entry;
    }

    private static CachedInfo GetCachedInfo(CacheEntry entry, BundleDetails details)
    {
    ...
    private static ReturnCode Run(TaskInput input, out TaskOutput output)
    {
        ...
        using (input.Log.ScopedStep(LogLevel.Info, "CopyingCachedFiles"))
        {
            foreach (ArchiveWorkItem item in cachedItems)
            {
                if (input.ProgressTracker != null &&
                    !input.ProgressTracker.UpdateInfoUnchecked(string.Format("{0} (Cached)", item.BundleName)))
        ...
        if (!ArchiveItems(nonCachedItems, input.TempOutputFolder, input.ProgressTracker, input.Threaded, input.Log, input.StripUnityVersion, allItems.Count, cachedItems.Count))
            return ReturnCode.Canceled;
        ...
        if (input.BuildCache != null)
        {
            using (input.Log.ScopedStep(LogLevel.Info, "Copying To Cache"))
            {
                List<CachedInfo> uncachedInfo = nonCachedItems
                    .Select(item => GetCachedInfo(cacheEntries[item.Index], item.ResultDetails))
                    .ToList();
        ...
        Dictionary<string, string> filenameToBundleName)
    {
        ...
        foreach (List<string> files in assetFileList)
        {
            ...
            bundleDependenciesHash.GetOrAdd(bundle, out HashSet<string> dependencies);
