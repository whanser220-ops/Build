...
public static class AngryMeshAddressablesReferenceConverter
{
    private const string GameAssetsRoot = "Assets/GameAssets/";
    private const string LegacyAngryMeshRoot = "Assets/ANGRY MESH/";
    private const string DefaultConvertedScenePath = "Assets/Scenes/Scenes 1/Scene_MeadowEnvironment_01_Summer_Addressables.unity";

    [MenuItem("Tools/ANGRY MESH/Addressables/Convert Enabled Scene Prefabs To Addressable Loaders")]
    public static void ConvertEnabledScenePrefabsFromMenu()
    {
        ConvertFromArgs(new[] { "--addressables-convert-enabled-scenes" });
    }
    ...
    {
        bool dryRun = HasArgument(args, "--addressables-convert-dry-run");
        bool enabledScenes = HasArgument(args, "--addressables-convert-enabled-scenes") ||
                             !HasArgument(args, "--addressables-convert-scene");
        bool updateBuildSettings = HasArgument(args, "--addressables-convert-update-build-settings");
        int limit = ParseInt(GetArgumentValue(args, "--addressables-convert-limit"), int.MaxValue);

        List<string> sourceScenes = new List<string>();
        string explicitScene = NormalizeAssetPath(GetArgumentValue(args, "--addressables-convert-scene"));
        ...
        {
            string destinationScene = NormalizeAssetPath(GetArgumentValue(args, "--addressables-convert-copy-to"));
    ...
    private static List<GameObject> CollectConvertiblePrefabRoots(Scene scene, SceneConversionResult result)
    {
        ...
        for (int index = 0; index < allObjects.Length; index++)
        {
            ...
            if (!IsConvertibleAddressableAssetPath(assetPath))
                continue;
    ...
    private static bool IsConvertibleAddressableAssetPath(string assetPath)
    {
    ...
    private static void ReplacePrefabInstanceWithLoader(GameObject prefabRoot, string address)
    {
        ...
        AngryMeshAddressablePrefabInstance loader = placeholder.AddComponent<AngryMeshAddressablePrefabInstance>();
    ...
    private static void LogResults(IReadOnlyList<SceneConversionResult> results, bool dryRun)
    {
        ...
        builder.AppendLine("ANGRY MESH Addressables reference conversion " + (dryRun ? "dry run" : "finished") + ".");
