...
当某条 `Directory` 规则位于另一条目录规则的下级路径时，面板会在继承自上级目录规则的字段左侧显示黄色小方块；未在当前规则勾选覆盖时，右侧会显示继承值。鼠标悬停在黄色标记上可以看到来源规则、规则类型和继承值。规则标题中也会显示 `Type` 标签，用来区分 `Package Name`、`Directory`、`Asset Class` 和兜底规则。
...
3. `Package Name` 非 `Any` 的规则优先级最高；其中 `Equals` 精确名称规则高于其他命名语义规则。
4. `Directory` 非 `Any` 的规则次之；目录 pattern 越具体，优先级越高。
5. 仅 `Asset Class` 命中的规则作为资产类型默认设置；全 `Any` 规则只作为兜底。
6. 同时包含 `Package Name` 和 `Directory` 条件的规则仍归入 `Package Name` 层，并用目录具体程度作为同层内的补充排序。
7. 高优先级规则只覆盖自己声明的属性；未声明的属性继续来自低优先级规则。
8. 同优先级规则对同一属性设置相同值时合并来源；设置不同值时视为规则冲突。
9. 存在规则冲突时，预览和检查/应用报告会列出冲突，自动导入与手动 Apply 都会跳过该资产，不做部分写入。
