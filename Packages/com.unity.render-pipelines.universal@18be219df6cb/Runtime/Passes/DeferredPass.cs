    ...
    internal class DeferredPass : ScriptableRenderPass
    {
        ...
        internal void Render(RenderGraph renderGraph, ContextContainer frameData, TextureHandle color, TextureHandle depth, TextureHandle[] gbuffer)
        {
            ...
            using (var builder = renderGraph.AddRasterRenderPass<PassData>(passName, out var passData, profilingSampler))
            {
                ...
                {
                    CommandBuffer cmd = new CommandBuffer { name = "Copy GBuffer0" };
                    int tempRT = Shader.PropertyToID("_TempGBuffer0");
                    cmd.GetTemporaryRT(tempRT, cameraData.camera.pixelWidth, cameraData.camera.pixelHeight, 0, FilterMode.Bilinear, RenderTextureFormat.ARGB32);
                    cmd.Blit(data.gbuffer[0], tempRT);  // 复制GBuffer0到临时RT
                    cmd.SetGlobalTexture("_CopiedGBuffer0", tempRT);  // 在Shader中用这个采样
                    cmd.ReleaseTemporaryRT(tempRT);  // 清理
